describe('Testa Home Page da aplicação', () => {
  ...
  it('Verifica se a página contém um input de busca de produtos', () => {
    ...
    cy.get('[data-testid="button-clear-search"]').should('exist');
  ...
  it('Verifica se o ao digitar no input de busca um nome inexistente, uma mensagem aparece na tela', () => {
    ...
    cy.wait(3000);
    cy.get('p').should('contain', 'Nenhum produto encontrado nesta busca!');
  })

  it('Verifica se a página contém uma lista de 16 produtos', () => {
    cy.visit('http://localhost:3000/');
    cy.wait(10000);
  ...
  //   cy.get('[data-testid="cart-button"]').click();
  //   // cy.get('[data-testid="cart-offCanvas"]').should('exist');
  //   // cy.get('[data-testid="list-products"]').should('exist');
  //   // cy.get('[data-testid="list-products"]').should('have.length', 1);
  //   // cy.get('[data-testid="cart-offCanvas"]').click();
  // })
